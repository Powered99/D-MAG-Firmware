...
#define STATION_NAME "HvF-BS 3G-MAG-1"
#define IAGA_CODE "HVF"
// Header geodetic location
...
namespace formats {
    ...
    extern bool sensor_reported[fgm::SENSOR_CH_COUNT];
    extern uint8_t reported_sensor_count;
...
namespace formats::IAGA2002{
    ...
    static header_line_t HEADER[HEADER_ENTRY_COUNT] = {
        ...
        make_header_entry("Reported", get_reported),
    ...
    static header_line_t COMMENTS[COMMENT_ENTRY_COUNT] = {
        ...
        make_header_comment("Sensor count: %s", formats::get_str_magnetometer_count),
        make_header_comment("Sensor channels: %s", formats::get_element_channels),
...
namespace formats::DMAG2026{
    const uint8_t header_entry_name_length = 22; // Header entry name (max) length
    const uint8_t header_entry_value_length = 48; // Header entry value max length
    const uint8_t header_line_width = 70; // Maximum header line width
    const uint8_t header_content_width = header_line_width - 1; // PIPE + content inside of the 70 character limit, so 69 characters for content.
    const uint8_t data_line_width = 128; // Maximum data line width
    ...
    const uint8_t HEADER_ENTRY_COUNT = 11;
    const uint8_t COMMENT_ENTRY_COUNT = 8;
    ...
    static header_line_t HEADER[HEADER_ENTRY_COUNT] = {
        ...
        make_header_entry("Sensor Orientation", SENSOR_ORIENTATION),
        make_header_entry("Data Type", DATA_TYPE),
        make_header_entry("Log Start", formats::get_datetime)
    };
    static header_line_t COMMENTS[COMMENT_ENTRY_COUNT] = {
        make_header_comment("Units: X (north+)[nT]"),
        make_header_comment("Accuracy: 1nT"),
